// Shared persistence for shopify_orders rows.
//
// Called from two places:
//   1. The order webhook (/api/webhooks/shopify/order) — live writes per order
//   2. The historical backfill (/api/identities/backfill-shopify) — bulk
//      catch-up for orders that pre-date the webhook registration
//
// The two callers receive slightly different shapes (webhook payload vs
// Admin REST API list response) but both expose the same fields we care
// about, so the input type is intentionally loose.
...
export interface PersistableShopifyOrder {
  id: number;
  name?: string | null;
  created_at: string;
...
// Best-effort US E.164 normalization. Shopify phones are messy.
...
export function pickPhone(order: PersistableShopifyOrder): string | null {
  return (
...
export async function persistShopifyOrder(
  ...
  const sql = getSql();
  const resolvedAttrs = attrs ?? extractAttrs(order);
  const resolvedPhone = phone ?? normalizePhoneE164(pickPhone(order));
  ...
  const identityId = await resolveCustomerIdentity({
    ...
    channel: "shopify",
    seen_at: orderedAt,
    gclid: resolvedAttrs.gclid,
    gbraid: resolvedAttrs.gbraid,
    wbraid: resolvedAttrs.wbraid,
    utm_source: resolvedAttrs.utm_source,
    utm_medium: resolvedAttrs.utm_medium,
    utm_campaign: resolvedAttrs.utm_campaign,
    landing_page: resolvedAttrs.landing_page ?? order.landing_site ?? null,
    order_value: total,
  });
  ...
  await sql`
    insert into shopify_orders (
      ...
      ${order.id}, ${order.name ?? null}, ${email}, ${resolvedPhone},
      ${order.customer?.id ?? null}, ${ordersCount}, ${isNew},
      ${orderedAt.toISOString()}, ${total}, ${order.currency ?? "USD"},
      ${resolvedAttrs.gclid}, ${resolvedAttrs.gbraid}, ${resolvedAttrs.wbraid},
      ${resolvedAttrs.fbclid}, ${resolvedAttrs.msclkid}, ${resolvedAttrs.ttclid},
      ${resolvedAttrs.utm_source}, ${resolvedAttrs.utm_medium}, ${resolvedAttrs.utm_campaign},
      ${resolvedAttrs.utm_term}, ${resolvedAttrs.utm_content},
      ${resolvedAttrs.landing_page ?? order.landing_site ?? null},
      ${resolvedAttrs.referrer ?? order.referring_site ?? null},
