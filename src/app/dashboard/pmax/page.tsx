...
export default function PMAXPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/google-ads/campaigns?year=${year}`, { cache: "no-store" })
      ...
      .then((d) => {
        ...
        // Filter to PMAX and Shopping only
        const filtered = d.map((m: MonthData) => ({
          ...m,
          campaigns: m.campaigns.filter((c) => c.type === "PMAX" || c.type === "Shopping"),
          totalSpend: m.campaigns.filter((c) => c.type === "PMAX" || c.type === "Shopping").reduce((s: number, c: Campaign) => s + c.spend, 0),
          totalConvValue: m.campaigns.filter((c) => c.type === "PMAX" || c.type === "Shopping").reduce((s: number, c: Campaign) => s + c.convValue, 0),
          totalClicks: m.campaigns.filter((c) => c.type === "PMAX" || c.type === "Shopping").reduce((s: number, c: Campaign) => s + c.clicks, 0),
  ...
  // Campaign-level totals across all months
      ...
      {!loading && !error && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <MetricCard label={`${year} Total Spend`} value={fmt(totalSpend)} subtext="PMAX + Shopping" highlight />
          ...
          {/* Campaign totals for the year */}
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden mb-6">
            <div className="px-6 py-4 border-b border-gray-800">
              <h2 className="text-sm font-semibold text-white">{year} Campaign Summary</h2>
            </div>
                ...
                {campaigns.map((c) => (
                  <tr key={c.name} className="text-gray-300 hover:bg-gray-800/40">
                    ...
                    <td className="py-2.5 px-4">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${c.type === "PMAX" ? "bg-blue-900/40 text-blue-400" : "bg-green-900/40 text-green-400"}`}>
                        {c.type}
                      </span>
                    </td>
                            ...
                            <tbody className="divide-y divide-gray-700/50">
                              {row.campaigns.map((c) => (
                                <tr key={c.name} className="text-gray-300">
                                  ...
                                  <td className="py-1.5">
                                    <span className={`px-2 py-0.5 rounded-full ${c.type === "PMAX" ? "bg-blue-900/40 text-blue-400" : "bg-green-900/40 text-green-400"}`}>
                                      {c.type}
                                    </span>
                                  </td>
