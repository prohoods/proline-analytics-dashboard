...
interface AmazonRow { month: string; campaign: string; type: string; impressions: number; clicks: number; cost: number; orders: number; revenue: number; roas: number; ctr: number; cpc: number; acos: number; }
interface Totals { impressions: number; clicks: number; cost: number; orders: number; revenue: number; roas: number; ctr: number; cpc: number; acos: number; }
interface ApiData { rows: AmazonRow[]; totals: Totals; months: string[]; }
...
const acosColor = (a: number) => a <= 15 ? "text-green-400" : a <= 25 ? "text-yellow-400" : "text-red-400";
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-400 mt-1">Sponsored Products, Brands & Display — monthly performance</p>
      ...
      {!loading && !error && data && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <MetricCard label="ACoS" value={`${data.totals.acos.toFixed(1)}%`} subtext="Ad Cost of Sales" trend={data.totals.acos <= 20 ? "up" : "down"} />
          </div>
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {data.rows.length === 0 ? (
                  <tr><td colSpan={9} className="py-8 text-center text-gray-500">No data yet — add rows to the Amazon Ads tab in your ad data sheet</td></tr>
                ) : data.rows.map((r, i) => (
                  <tr key={i} className="text-gray-300 hover:bg-gray-800/40">
                    ...
                    <td className="py-2.5 px-4"><span className="text-xs px-2 py-0.5 rounded-full bg-orange-900/40 text-orange-400">{r.type}</span></td>
                    ...
                    <td className={`py-2.5 px-4 text-right font-semibold ${acosColor(r.acos)}`}>{r.acos > 0 ? `${r.acos.toFixed(1)}%` : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
