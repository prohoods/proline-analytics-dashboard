...
interface PinterestRow { month: string; impressions: number; clicks: number; cost: number; }
interface ApiData { rows: PinterestRow[]; totals: { cost: number; impressions: number; clicks: number }; }
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-400 mt-1">Pinterest Ads — monthly spend</p>
      ...
      {error && (
        <div className="text-red-400 bg-red-900/20 rounded-lg p-4 mb-6 text-sm">
          ...
          {error.includes("parse range") && (
            <p className="mt-2 text-yellow-400">Check that the sheet tab is named exactly <strong>Pintrest</strong> (as spelled in your sheet).</p>
          )}
        </div>
      )}
        ...
        <>
          <div className="grid grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              ...
              <div className="text-2xl font-bold text-white">{fmtN(data.totals.impressions)}</div>
            </div>
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {data.rows.length === 0 ? (
                  <tr><td colSpan={5} className="py-8 text-center text-gray-500">No data — add rows to the Pintrest tab</td></tr>
                ) : data.rows.map((r, i) => (
              ...
              <tfoot>
                <tr className="bg-gray-800/50 border-t border-gray-700 font-semibold text-white text-xs">
                  ...
                  <td className="py-3 px-4 text-right text-gray-300">{fmtN(data.totals.impressions)}</td>
