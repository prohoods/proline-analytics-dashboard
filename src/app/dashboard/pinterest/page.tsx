...
interface PinterestRow { month: string; campaign: string; impressions: number; clicks: number; cost: number; conversions: number; revenue: number; roas: number; ctr: number; cpc: number; }
...
interface ApiData { rows: PinterestRow[]; totals: Totals; months: string[]; }
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-400 mt-1">Pinterest Ads — monthly campaign performance</p>
      ...
      {!loading && !error && data && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <MetricCard label="Impressions" value={fmtN(data.totals.impressions)} subtext={`${data.totals.ctr.toFixed(2)}% CTR`} />
          </div>
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {data.rows.length === 0 ? (
                  <tr><td colSpan={9} className="py-8 text-center text-gray-500">No data yet — add rows to the Pinterest tab in your ad data sheet</td></tr>
                ) : data.rows.map((r, i) => (
