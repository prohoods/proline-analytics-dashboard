...
export default function BingPmaxPage() {
  ...
  useEffect(() => {
    fetch("/api/sheets/bing")
      .then(r => r.json())
      .then(d => { if (d.error) throw new Error(d.error); setRows(d.rows.filter((r: BingRow) => r.type === "PMAX")); setLoading(false); })
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">Bing — PMAX Campaigns</h1>
        <p className="text-gray-400 mt-1">Performance Max campaigns on Microsoft Advertising</p>
        ...
        <>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-white">PMAX by Month</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {rows.length === 0 ? (
                  <tr><td colSpan={9} className="py-8 text-center text-gray-500">No PMAX data — add rows with Type = PMAX in the Bing Ads sheet tab</td></tr>
                ) : rows.map((r, i) => (
