...
export async function GET() {
  try {
    const rows = await getSheetData(SHEET_ID, "Pinterest!A2:G2000");

    const data = rows
      .filter(r => r.length >= 2 && r[0])
      .map(r => {
        const impressions = parseNum(r[2]);
        const clicks = parseNum(r[3]);
        const cost = parseNum(r[4]);
        const conversions = parseNum(r[5]);
        const revenue = parseNum(r[6]);
        return {
          month: r[0] ?? "",
          campaign: r[1] ?? "",
    ...
    const conv = data.reduce((s, r) => s + r.conversions, 0);
    const rev = data.reduce((s, r) => s + r.revenue, 0);
    const months = Array.from(new Set(data.map(r => r.month))).sort().reverse();

    return NextResponse.json({
      rows: data,
      totals: { impressions: imp, clicks: clk, cost, conversions: conv, revenue: rev,
